...
export function FinanceSimpleConfigPage({
  ...
  async function addRow() {
    ...
    try {
      ...
      if (body && typeof body === "object" && body.id) {
        setRows((prev) => [...prev, body as Row].sort((a, b) => a.name.localeCompare(b.name, "pt-BR")));
      } else {
  ...
  async function deleteRow(row: Row) {
    if (!window.confirm(`Excluir "${row.name}"? O item será removido da lista.`)) {
      return;
    }
    setDeletingId(row.id);
    setError(null);
    try {
      const r = await apiFetch(`${apiPath}/${row.id}`, { method: "DELETE" });
  ...
  async function saveEdit(row: Row) {
    ...
    try {
      ...
      if (body && typeof body === "object" && body.id) {
        setRows((prev) =>
          prev
            .map((x) => (x.id === row.id ? { ...x, ...(body as Row) } : x))
