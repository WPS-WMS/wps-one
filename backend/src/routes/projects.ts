...
projectsRouter.get("/", async (req, res) => {
  ...
  if (lightMode) {
    ...
    const lightweight = projectsLight.map((project) => {
      const summary = byProject.get(project.id) ?? { totalTopicos: 0, totalTarefas: 0, finalizadas: 0, atrasadas: 0 };
      return {
        ...project,
        tickets: [],
        listMode: "summary" as const,
...
projectsRouter.get("/:id/proposal", async (req, res) => {
  ...
  if (download) {
    ...
    try {
      res.setHeader("Content-Disposition", contentDispositionAttachment(name, "proposta-comercial"));
    } catch (err) {
      // Nunca derrubar o processo por nome de arquivo inválido
      console.error("[projects] Content-Disposition inválido", { name, err });
